...
import {
	...
	hideScreenWhenShippingAddressUpdated
...
export function listenShippingContainerEvents() {
	...
	/**
	* Listen to shipping address change event.
	*
	* @return {void} void.
	* @since 1.2.2
	*/
	document.addEventListener("addonifyFloatingCartShippingAddressUpdated", () => {
		if (hideScreenWhenShippingAddressUpdated) {
			setTimeout(() => {
				$("#adfy__woofc-shipping-container").attr("data_display", "hidden");
			}, 1000);
		}
...
export function handleShippingMethodChange() {
	$(document).on("change", ".shipping_method", function (e) {
		...
		let message = __(
			"Error processing shipping method update request.",
			"addonify-floating-cart"
		);
...
function populateStatesOnceCountryIsChanged() {
	$(document).on(
		"change",
		"#addonify_floating_cart_shipping_country",
		function (e) {
			...
			if (typeof states === "object" && Object.keys(states).length > 0) {
				...
				for (let index in states) {
					html +=
						'<option value="' + index + '">' + states[index] + "</option>";
				}
				...
				if (stateDiv.prop("tagName").toLowerCase() === "input") {
					...
					select
						...
						.prop("id", "addonify_floating_cart_shipping_state")
						.prop("name", "addonify_floating_cart_shipping_state");
